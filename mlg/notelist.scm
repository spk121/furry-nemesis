        ...
        (let loop ([remainingTicks tick]
          ...
          (if (>= tempoIdx (length tempoMap))
              ...
              (let* ([lastTempo (cdr (list-ref tempoMap (1- (length tempoMap))))]
                ...
                (+ timeInSeconds (* remainingTicks msPerTick 1e-6)))
              ;; Check if there's a tempo change before we reach the target tick
              (let* ([currentTempoEntry (list-ref tempoMap tempoIdx)]
                     [currentTempoTick (car currentTempoEntry)]
                     [currentTempoValue (cdr currentTempoEntry)])
                (if (and (< tempoIdx (1- (length tempoMap)))
                         (< (car (list-ref tempoMap (1+ tempoIdx))) tick))
                    ;; There's a tempo change before target, process segment
                    (let* ([nextTempoTick (car (list-ref tempoMap (1+ tempoIdx)))]
                           [ticksInSegment (- nextTempoTick currentTempoTick)]
                           [msPerTick (exact->inexact (/ currentTempoValue ticksPerQuarterNote))]
                           [segmentTime (* ticksInSegment msPerTick 1e-6)])
                      (loop (- remainingTicks ticksInSegment)
                            (+ timeInSeconds segmentTime)
                            (1+ tempoIdx)))
                    ;; No more tempo changes before target, use current tempo
                    (let* ([msPerTick (exact->inexact (/ currentTempoValue ticksPerQuarterNote))]
                           [segmentTime (* remainingTicks msPerTick 1e-6)])
                      (+ timeInSeconds segmentTime))))))))
